# Olá, eu sou o Erick Vieira 👋

> Cientista de Dados formado pela Anhanguera | Graduando em IA e Machine Learning @ UNIASSELVI
> Focado em **Visão Computacional** e **Desenvolvimento de Aplicações Inteligentes**

📍 Piedade - SP, Brasil | 💼 Buscando oportunidade como Dev / Cientista de Dados / CV Engineer

### 🚀 Sobre mim
Sou apaixonado por transformar imagens em inteligência. Estudo IA/ML com foco em Visão Computacional, unindo base sólida em Ciência de Dados com Python e matemática aplicada. Gosto de teoria + prática: aprendo um conceito e já coloco no GitHub.

### 🧠 Tech Stack
**Linguagens:** Python, SQL
**Visão Computacional:** OpenCV, YOLOv8, PyTorch, TensorFlow, MediaPipe
**Dados & ML:** Scikit-learn, Pandas, NumPy, Matplotlib
**Deploy & Apps:** FastAPI, Streamlit, Docker, Git

### 🔭 Projetos em construção (Portfólio CV)
- **[Projeto 1] Classificador Inteligente** - Transfer Learning com EfficientNet | `em desenvolvimento`
- **[Projeto 2] Detector em Tempo Real** - YOLOv8 para detecção de objetos | `próximo`
- **[Projeto 3] App de Visão Completo** - API + Interface Web | `planejado`

### 📊 GitHub Stats
![Erick's GitHub stats](https://github-readme-stats.vercel.app/api?username=oerickvieira&show_icons=true&theme=tokyonight)
![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=oerickvieira&layout=compact&theme=tokyonight)

### 📫 Contato
- LinkedIn: [seu link aqui]
- Email: [seu email aqui]

---
⭐ *"Teoria te diz o que é possível. Código te mostra como fazer."*

<!--
**oerickvieira/oerickvieira** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
